...
class BoardStorage {
  static async savePhoto(contents, id) {
    return new Promise((resolve, reject) => {
      const query =
        "INSERT INTO photo(photo_like_cnt, photo_id, photo_title, photo_description) VALUES(?, ?, ?, ?);";
      db.query(query, [0, id, contents.title, contents.description], (err) => {
        if (err) reject(`${err}`);
  ...
  static async saveVideo(contents, id) {
    return new Promise((resolve, reject) => {
      const query =
        "INSERT INTO video(video_like_cnt, video_id, video_title, video_description) VALUES(?, ?, ?, ?);";
      db.query(query, [0, id, contents.title, contents.description], (err) => {
        if (err) reject(`${err}`);
        resolve({ success: true });
      });
  ...
  static async saveQuestion(contents, id, upload_time) {
    return new Promise((resolve, reject) => {
      const query =
        "INSERT INTO question(question_id, question_title, question_description, upload_time) VALUES(?, ?, ?, ?);";
      db.query(
        query,
  ...
  static async addPoint(pt, id, pluspoint) {
    ...
    db.query(query, [pt + pluspoint, id], (err) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async removePoint(pt, id, minuspoint) {
    const query = "UPDATE member SET point=? WHERE memberid = ?";
    db.query(query, [pt - minuspoint, id], (err) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async deleteoverlapPhoto(delete_overlap) {
    ...
    db.query(query, [del_overlap], (err) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async updateoverlapPhoto(current_overlap, title, update_cnt) {
    return new Promise((resolve, reject) => {
      ...
      db.query(query, [current_overlap, title + update_cnt], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
    });
  ...
  static async updatecommentoverlapPhoto(current_overlap, title, update_cnt) {
    return new Promise((resolve, reject) => {
      const query =
        "UPDATE photo_comment SET overlap = REPLACE(overlap, ?, ?);";
      db.query(query, [current_overlap, title + update_cnt], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
  ...
  static async updatecommentoverlapVideo(current_overlap, title, update_cnt) {
    return new Promise((resolve, reject) => {
      const query =
        "UPDATE video_comment SET overlap = REPLACE(overlap, ?, ?);";
      db.query(query, [current_overlap, title + update_cnt], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
  ...
  static async updatecommentoverlapQuestion(
    current_overlap,
    title,
    update_cnt
  ) {
    return new Promise((resolve, reject) => {
      const query =
        "UPDATE question_comment SET overlap = REPLACE(overlap, ?, ?);";
      db.query(query, [current_overlap, title + update_cnt], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
  ...
  static async photolikeProduce(id, title, check_seq) {
    const query =
      "INSERT INTO photo_like(user_id, photo_title, overlap) VALUES(?, ?, ?) ON DUPLICATE KEY UPDATE user_id = VALUES(user_id), photo_title = VALUES(photo_title);";
    db.query(query, [id, title, id + title + check_seq], (err) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async videolikeProduce(id, title, check_seq) {
    const query =
      "INSERT INTO video_like(user_id, video_title, overlap, like_check) VALUES(?, ?, ?, ?) ON DUPLICATE KEY UPDATE user_id = VALUES(user_id), video_title = VALUES(video_title);";
    db.query(query, [id, title, id + title + check_seq, 2], (err) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async photocountUpdate(seq, like_cnt, check) {
    ...
    if (check == 1) {
      db.query(query, [like_cnt + 1, seq], (err) => {
        if (err) reject(`${err}`);
      });
    } else {
      db.query(query, [like_cnt - 1, seq], (err) => {
  ...
  static async photodeClaration(seq, declaration) {
    ...
    db.query(query, [declaration + 1, seq], (err) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async uploadcommentPhoto(
    user_id,
    overlap,
    photo_comment,
    photo_comment_nickname,
    upload_time
  ) {
    return new Promise((resolve, reject) => {
      const query =
        "INSERT INTO photo_comment(user_id, overlap, photo_comments, nickname, upload_time) VALUES(?, ?, ?, ?, ?);";
      db.query(
        query,
  ...
  static async uploadcommentVideo(
    user_id,
    overlap,
    video_comment,
    video_comment_nickname,
    upload_time
  ) {
    return new Promise((resolve, reject) => {
      const query =
        "INSERT INTO video_comment(user_id, overlap, video_comments, nickname, upload_time) VALUES(?, ?, ?, ?, ?);";
      db.query(
        query,
  ...
  static async uploadcommentQuestion(
    user_id,
    overlap,
    question_comment,
    question_comment_nickname,
    upload_time
  ) {
    return new Promise((resolve, reject) => {
      const query =
        "INSERT INTO question_comment(user_id, overlap, question_comments, nickname, upload_time) VALUES(?, ?, ?, ?, ?);";
      db.query(
        query,
        [
          user_id,
  ...
  static async updatecommentPhoto(update_seq, update_comment) {
    ...
    db.query(query, [update_comment, update_seq], (err) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async updatecommentQuestion(update_seq, update_comment) {
    const query =
      "UPDATE question_comment SET question_comments = ? WHERE seq = ?";
    db.query(query, [update_comment, update_seq], (err) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async commentphotoCount(overlap) {
    return new Promise((resolve, reject) => {
      const query =
        "SELECT COUNT(overlap) as cnt FROM photo_comment WHERE overlap LIKE ?;";
  ...
  static async commentvideoCount(overlap) {
    return new Promise((resolve, reject) => {
      const query =
        "SELECT COUNT(overlap) as cnt FROM video_comment WHERE overlap LIKE ?;";
      db.query(query, [overlap], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0].cnt);
      });
  ...
  static async commentQuestionCount(overlap) {
    return new Promise((resolve, reject) => {
      const query =
        "SELECT COUNT(overlap) as cnt FROM question_comment WHERE overlap LIKE ?;";
      db.query(query, [overlap], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0].cnt);
      });
  ...
  static async photocommentRenew(overlap) {
    const query =
      "UPDATE photo_comment SET photo_comment_check = 0 WHERE overlap LIKE ?;";
    db.query(query, [overlap], (err, data) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async videocommentRenew(overlap) {
    const query =
      "UPDATE video_comment SET video_comment_check = 0 WHERE overlap LIKE ?;";
    db.query(query, [overlap], (err, data) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async questioncommentRenew(overlap) {
    const query =
      "UPDATE question_comment SET question_comment_check = 0 WHERE overlap LIKE ?;";
    db.query(query, [overlap], (err, data) => {
      if (err) reject(`${err}`);
    });
  }
  ...
  static async commentgetIdPhoto(overlap) {
    return new Promise((resolve, reject) => {
      const query =
        "SELECT * FROM photo_comment WHERE photo_comment_check = 0 and overlap LIKE ?;";
      db.query(query, [overlap], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
  ...
  static async commentgetIdVideo(overlap) {
    return new Promise((resolve, reject) => {
      const query =
        "SELECT * FROM video_comment WHERE video_comment_check = 0 and overlap LIKE ?;";
      db.query(query, [overlap], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
  ...
  static async commentgetIdQuestion(overlap) {
    return new Promise((resolve, reject) => {
      const query =
        "SELECT * FROM question_comment WHERE question_comment_check = 0 and overlap LIKE ?;";
      db.query(query, [overlap], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
  ...
  static async commentcheckpUpdate(seq, overlap) {
    return new Promise((resolve, reject) => {
      const query =
        "UPDATE photo_comment SET photo_comment_check = 1 WHERE seq = ? and overlap LIKE ?;";
      db.query(query, [seq, overlap], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
    });
  ...
  static async commentcheckvUpdate(seq, overlap) {
    return new Promise((resolve, reject) => {
      const query =
        "UPDATE video_comment SET video_comment_check = 1 WHERE seq = ? and overlap LIKE ?;";
      db.query(query, [seq, overlap], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
  ...
  static async commentcheckqUpdate(seq, overlap) {
    return new Promise((resolve, reject) => {
      const query =
        "UPDATE question_comment SET question_comment_check = 1 WHERE seq = ? and overlap LIKE ?;";
      db.query(query, [seq, overlap], (err, data) => {
        if (err) reject(`${err}`);
        else resolve(data[0]);
      });
  ...
  static async addAnswer(question_num, seq) {
    ...
    db.query(query, [question_num + 1, seq], (err) => {
      if (err) reject(`${err}`);
    });
  }
}
