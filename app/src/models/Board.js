...
class Board {
  ...
  async photoSearchTitledesLike(seq) {
    const { photo_like_cnt, photo_title, photo_description, photo_id } =
      await UserStorage.photogetDescription(seq);
    return [photo_like_cnt, photo_title, photo_description, photo_id];
  }

  async videoSearchTitledesLike(seq) {
    const { video_like_cnt, video_title, video_description, video_id } =
      await UserStorage.videogetDescription(seq);
  ...
  async questionSearchTitledesLike(seq) {
    const {
      ...
      question_num,
      upload_time,
    } = await UserStorage.questiongetDescription(seq);
    return [
      question_num,
      question_title,
      question_description,
  ...
  async photocommentoverlapUpdate(title, current_cnt, update_cnt) {
    var current_overlap = title + current_cnt;
    await UserStorage.updatecommentoverlapPhoto(
      current_overlap,
      title,
      update_cnt
    );
  ...
  async videocommentoverlapUpdate(title, current_cnt, update_cnt) {
    var current_overlap = title + current_cnt;
    await UserStorage.updatecommentoverlapVideo(
      current_overlap,
      title,
      update_cnt
    );
  ...
  async questioncommentoverlapUpdate(title, current_cnt, update_cnt) {
    var current_overlap = title + current_cnt;
    await UserStorage.updatecommentoverlapQuestion(
      current_overlap,
      title,
      update_cnt
    );
  ...
  async photoUpatelike(photo_seq, check) {
    const { seq, photo_like_cnt } = await UserStorage.photogetDescription(
      photo_seq
    );
    if (check == 1) {
  ...
  async videoUpatelike(video_seq, check) {
    const { seq, video_like_cnt } = await UserStorage.videogetDescription(
      video_seq
    );
    if (check == 3) {
  ...
  async photodeclarationUpdate(photo_seq) {
    const { seq, photo_declaration } = await UserStorage.photogetDescription(
      photo_seq
    );
  ...
  async videodeclarationUpdate(photo_seq) {
    const { seq, video_declaration } = await UserStorage.videogetDescription(
      photo_seq
    );
  ...
  async questiondeclarationUpdate(photo_seq) {
    const { seq, question_declaration } =
      await UserStorage.questiongetDescription(photo_seq);
  ...
  async photocommentUpload(
    ...
    try {
      const response = await UserStorage.uploadcommentPhoto(
        user_id,
        overlap,
        photo_comment,
        photo_comment_nickname,
  ...
  async videocommentUpload(
    ...
    try {
      const response = await UserStorage.uploadcommentVideo(
        user_id,
        overlap,
        video_comment,
        video_comment_nickname,
  ...
  async questioncommentUpload(
    ...
    try {
      const response = await UserStorage.uploadcommentQuestion(
        user_id,
        overlap,
        question_comment,
        question_comment_nickname,
  ...
  async photocommentgetId(overlap) {
    const { seq, user_id, photo_comments, nickname, upload_time } =
      await UserStorage.commentgetIdPhoto(overlap);
  ...
  async videocommentgetId(overlap) {
    const { seq, user_id, video_comments, nickname, upload_time } =
      await UserStorage.commentgetIdVideo(overlap);
  ...
  async questioncommentgetId(overlap) {
    const { seq, user_id, question_comments, nickname, upload_time } =
      await UserStorage.commentgetIdQuestion(overlap);
